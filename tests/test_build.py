...
EXPECTED_EDITION='player'
EXPECTED_REPOSITORY='dragons-gate-player-hud'
    ...
    def test_build_emits_verified_owned_package(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with zipfile.ZipFile(recovery) as z:
                ...
                self.assertIn('<version>1.7.0</version>',recovery_xml)
                self.assertIn(f"local runtime={{version=&#x27;1.7.0&#x27;,owner=&#x27;{owner}&#x27;,repository=&#x27;{repository}&#x27;}}",recovery_xml)
                ...
                self.assertIn('hud._view_handoff=nil',recovery_script)
                self.assertIn('controller.update_preserve_view=nil',recovery_script)
                self.assertNotIn('controller.update_preserve_view=true',recovery_script)
            ...
            for filename in ('view.lua','navigation.lua'):
                ...
                expected_contract.update(payload)
            self.assertEqual(manifest['view_contract'],expected_contract.hexdigest())
            ...
            with zipfile.ZipFile(package) as z:
                ...
                self.assertIn(f'view_contract = "{manifest["view_contract"]}"',runtime)
                self.assertNotIn('__DGHUD_VIEW_CONTRACT__',runtime)
