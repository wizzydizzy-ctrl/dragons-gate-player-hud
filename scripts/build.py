...
MODULES=['defaults','keybindings','command_parser','command_collector','game_clock','chat_parser','chat_history','chat_storage','chat_controller','chat_sounds','color_styles','color_preferences','travel_highlights','output_colorizer','posture_tracker','needs_tracker','autoroller','navigation','mapper_model','map_adapter','map_transfer','map_catalog','map_collections','map_cleanup','map_diagnostics','failure_report','automapper','special_transition','map_walker','state','settings','sha256','release','events','layout','view','mudlet_adapter','main','updater']
def source_version():
...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--output',type=Path,default=ROOT/'dist'); p.add_argument('--owner',default='wizzydizzy-ctrl'); p.add_argument('--repository',default='dragons-gate-player-hud'); p.add_argument('--version',default=source_version()); a=p.parse_args(); build(a.output,a.owner,a.repository,a.version)
