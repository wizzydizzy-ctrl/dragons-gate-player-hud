...
local LIMITS={maps=1000,slug=64,name=100,author=80,publisher=39,description=500,version=32,areas=100,area=100,rooms=100000}
local ENTRY_FIELDS={slug=true,name=true,author=true,publisher=true,description=true,version=true,areas=true,room_count=true,bytes=true,download_url=true,sha256=true}
...
local function validateEntry(entry,index)
  ...
  for key in pairs(ENTRY_FIELDS) do if entry[key]==nil then return fail("maps["..index.."] is missing "..key) end end
...
function Catalog.search(catalog,query)
  ...
  local matches={}
  for _,entry in ipairs(normalized.maps) do
    local haystack=(entry.name.."\n"..entry.author.."\n"..entry.publisher.."\n"..entry.description.."\n"..table.concat(entry.areas,"\n")):lower()
